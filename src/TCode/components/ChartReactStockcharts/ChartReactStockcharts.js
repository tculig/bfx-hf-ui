...
import { propTypes, defaultProps } from './ChartReactStockcharts.props'
import './style.scss'
import BasicVolumeCandle from '../ReactStockcharts/BasicVolumeCandle'

const HEIGHT_STEP_PX = 20
const MIN_HEIGHT_PX = 250

export default class ChartReactStockcharts extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    const {
      trades, positions, exchanges, orders, syncRanges, xExtentsCommon, activeMarket,
    } = this.props
    const {
    ...
    if (!_isEqual(nextProps.syncRanges, syncRanges)
      ...
      || (nextProps.xExtentsCommon !== xExtentsCommon)
      || (nextProps.activeMarket !== activeMarket)
    ) {
      return true
    }
  ...
  render() {
    const {
      updateXExtends, xExtentsCommon, height, activeMarket,
    } = this.props
    return (
      <div style={{
        display: 'flex',
